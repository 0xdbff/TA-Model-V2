...
- `ExecutionCostModel.taker_fee_rate` uses the same bounded fee-rate contract family as instrument fee schedules.
...
- `ReplayOrderResult` validates attribution integrity, including cost-model hash/id pairing, total-cost summation, and price/notional consistency even when IDs are rebuilt from malformed payloads.
...
- `tests/test_execution_cost_replay.py::test_sell_side_higher_stress_costs_lower_effective_price_and_raise_cost` covers sell-side adverse execution: higher spread/slippage lowers effective sell price while increasing positive cost attribution.
...
- `uv run pytest` — passed (`179 passed`).
