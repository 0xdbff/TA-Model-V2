...
- Added fail-closed validation for late-source bars, duplicate bars, globally unsorted event-time inputs, per-stream non-monotonic/overlapping bars, mixed-timeframe ambiguity, duplicate client order IDs, and unsorted order-intent `(submitted_at, client_order_id)` keys; missing future events produce explicit unfilled results.
...
- Fail-closed unsafe inputs: `tests/test_event_time_replay.py::test_duplicate_or_non_monotonic_bars_fail_closed` covers duplicate and unsorted bars; `tests/test_event_time_replay.py::test_mixed_timeframes_for_same_instrument_venue_fail_closed` covers ambiguous mixed timeframes; `tests/test_event_time_replay.py::test_duplicate_client_order_id_fails_closed` covers duplicate order intents; `tests/test_event_time_replay.py::test_non_monotonic_order_intent_submitted_at_fails_closed_before_replay` and `tests/test_event_time_replay.py::test_equal_timestamp_order_intents_are_sorted_by_client_order_id` cover causal order-intent input ordering.
...
- `uv run pytest` — passed, 215 tests in current integrated loop validation.
...
- Order-intent input ordering must be causal by `(submitted_at, client_order_id)` and fails closed before account-state mutation if non-monotonic.
