...
- Enforced when optional metadata/state are supplied:
  ...
  - active event-time instrument constraint via `InstrumentConstraint.evaluate_order` using the eligible fill bar open as the market-order reference price;
...
`ReplayReport.rejection_counts` is deterministic and included in `replay_report_hash` with `final_account_state`.

## Validation commands

- `uv run pytest tests/test_venue_account_replay.py tests/test_event_time_replay.py tests/test_execution_cost_replay.py` — 28 passed.
